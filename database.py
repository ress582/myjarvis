...
class Database:
    ...
    def reset_daily_nutrition(self) -> None:
        ...
        self._save_data()
        def add_notification(self, time: str, message: str, date: str = None) -> Dict:
            """Add a notification reminder."""
            if date is None:
                date = datetime.date.today().isoformat()
                
            notification = {
                "id": len(self.data.get("notifications", [])) + 1,
                "time": time,
                "date": date,
                "message": message,
                "triggered": False
        ...
        def parse_schedule_format(self, format_str: str) -> Dict:
            """Parse the AI's schedule format string."""
            try:
                parts = format_str.split('/')
                if len(parts) >= 4 and parts[0].lower() == 'schedule':
                    return {
                        "title": parts[1],
                        "date": datetime.datetime.strptime(parts[2], "%B %d, %Y").strftime("%Y-%m-%d"),
                        "time": datetime.datetime.strptime(parts[3].strip(), "%I:%M %p").strftime("%I:%M %p")
                    }
            except (ValueError, IndexError):
                pass
            return None
    
        def parse_notify_format(self, format_str: str) -> Dict:
            """Parse the AI's notify format string."""
            try:
                parts = format_str.split('/')
                if len(parts) >= 3 and parts[0].lower() == 'notify':
                    return {
                        "time": datetime.datetime.strptime(parts[1].strip(), "%I:%M %p").strftime("%I:%M %p"),
        ...
        def get_pending_notifications(self, current_time: datetime.datetime = None) -> List[Dict]:
            """Get all pending notifications that haven't been triggered yet."""
            ...
            for notif in self.data["notifications"]:
                if not notif["triggered"] and notif["date"] == current_date:
                    try:
                        notif_time = datetime.datetime.strptime(notif["time"], "%I:%M %p").time()
                        if current_time.time() >= notif_time:
        ...
        def mark_notification_triggered(self, notification_id: int) -> bool:
