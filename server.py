...
def ask():
    ...
    jaws_prompt = (
        ...
        f"1. SCHEDULING: When you detect any scheduling intent (appointments, meetings, events, reminders):\n"
        f"   - Extract name, date, time and description from the conversation\n"
        f"   - Format as: schedule/[name]/[date]/[time]/[description]\n"
        f"   - Example: schedule/Dentist Appointment/tomorrow/2:30 PM/Regular checkup\n\n"
        f"2. NOTIFICATIONS: When you detect a notification request:\n"
        f"   - Determine an appropriate notification time\n"
        f"   - Format as: notify/[time]/[message]\n"
        f"   - Example: notify/15:00/Time to prepare for the meeting\n\n"
        f"3. Always be proactive in suggesting helpful actions based on the context.\n"
